...
<html>
    ...
    <body>
		...
		$requestDetail  = $request->getRequestDetails($requestId,$loggedUserId);
		if( !$requestDetail )
		{
			header('Location: '.$user->root.'dashboard.php');
		}
					...
					<table class="table" id="dtableo">
					    ...
					    <?php if( !empty($assignedEmployees)){ 
							...
							<tr>
								...
								<td class="hireBtnTd"><?php if(!$isEmpHiredForReq) { if( $emp['status'] == 2 ) {?><p class="subir"><a data-requestId="<?php echo $requestId;?>" data-employeeId="<?php echo $emp['id'];?>" href="javascript:;" class="hireEmployee">Hire</a></p><?php } } ?></td>
							</tr>
						<?php $i++; } } else { ?>
